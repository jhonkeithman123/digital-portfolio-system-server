...
rrouter.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    try {
      // find classroom by code (no FOR UPDATE / transaction to avoid prepared-statement START TRANSACTION errors)
      ...
      // look for an existing classroom_members row for this student + classroom
      ...
      if (memberRows.length) {
        // there is a pending/invited row — accept it
        await queryAsync(
          "UPDATE classroom_members SET status = 'accepted' WHERE id = ? AND student_id = ?",
          [memberRows[0].id, studentId]
        );

        // cleanup any hidden_invites pointing to that invite id
        queryAsync(
          "DELETE FROM hidden_invites WHERE invite_id = ? AND student_id = ?",
          [memberRows[0].id, studentId]
        ).catch((e) => console.error("Error cleaning hidden invites:", e));
      } else {
        // insert a new accepted membership; handle rare race/duplicate gracefully
        try {
          ...
          // if there was an existing hidden_invite for a previous invite row, try to clean it.
          // find any classroom_members rows for this classroom+student (could be the inserted one or an earlier pending)
          const existing = await queryAsync(
          ...
          // handle unique/duplicate race: another process inserted concurrently — treat as accepted
          if (err && (err.code === "ER_DUP_ENTRY" || err.errno === 1062)) {
            // best-effort: mark existing row accepted
            try {
              const other = await queryAsync(
